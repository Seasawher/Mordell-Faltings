...
\renewcommand{\proofname}{\textgt{Proof}}%proof環境の修正
...
\newtheorem*{definition}{difinition}
...
\newtheorem*{quo}{quotation}%旧バージョン
%\newtheorem*{quo}{}%新バージョン
\newtheorem*{rem}{remark}
\newtheorem*{que}{query}%疑問
...
%\newcommand{\barquo}[1]{\begin{leftbar} \begin{quo} #1 \end{quo} \end{leftbar}}%左線つき引用%旧バージョン
\newcommand{\barquo}[1]{\begin{leftbar} \noindent #1  \end{leftbar}}%左線つき引用%新バージョン
...
\newcommand{\kakko}[1]{ \langle #1  \rangle}%角かっこ%\angleはもうある
...
\newcommand{\trform}[1]{ (#1)_{\Tr_{K/\Q}} }%代数体のQ上のトレース
\DeclareMathOperator{\vol}{vol}%ボリューム
...
\DeclareMathOperator{\Proj}{Proj}%射影空間版スペクトル
\DeclareMathOperator{\gr}{gr}%次数環のアレ
\DeclareMathOperator{\Sing}{Sing}%Singular point.特異点の集合。
...
\DeclareMathOperator{\Func}{Func}%関数
\DeclareMathOperator{\Pic}{Pic}%ぴかーる群
